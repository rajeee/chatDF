...
def build_system_prompt(datasets: list[dict]) -> str:
    ...
    if datasets:
        ...
        parts.append("")
        parts.append("After executing a SQL query, PROACTIVELY call create_chart when the results match these patterns:")
        parts.append("- 3+ rows with a categorical column and a numeric column → bar chart")
        parts.append("- Results ordered by date/time with a numeric column → line chart")
        parts.append("- Two numeric columns with 5+ rows → scatter plot")
        parts.append("- A single numeric column with 10+ distinct values → histogram")
        parts.append("- Proportions or percentages that sum to ~100% → pie chart (only <=8 categories)")
        parts.append("- User asks about 'trends', 'patterns', 'distribution', 'comparison', or 'breakdown' → always visualize")
        parts.append("Charts make data come alive — when in doubt, create the chart. Users love visual answers.")
        parts.append("")
