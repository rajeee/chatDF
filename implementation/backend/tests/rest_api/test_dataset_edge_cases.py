...
# ===========================================================================
# 2. Profile endpoint error handling
# ===========================================================================


@pytest.mark.asyncio
@pytest.mark.integration
async def test_profile_worker_returns_error_dict(
    authed_client, fresh_db, conversation_owned, dataset_in_conversation, mock_worker_pool
):
    """POST profile should return 500 when worker_pool.profile_columns returns error dict."""
    from app.main import app

    mock_worker_pool.profile_columns = AsyncMock(
        return_value={"error_type": "internal", "message": "Profiling crashed"}
    )
    app.state.worker_pool = mock_worker_pool

    dataset_id = dataset_in_conversation["id"]
    response = await authed_client.post(
        f"/conversations/{conversation_owned['id']}/datasets/{dataset_id}/profile",
    )
...
@pytest.mark.asyncio
@pytest.mark.integration
async def test_profile_worker_returns_error_key(
    authed_client, fresh_db, conversation_owned, dataset_in_conversation, mock_worker_pool
):
    """POST profile should return 500 when worker_pool.profile_columns returns {error: ...}."""
    from app.main import app

    mock_worker_pool.profile_columns = AsyncMock(
        return_value={"error": "Column stats computation failed"}
    )
    app.state.worker_pool = mock_worker_pool
...
@pytest.mark.asyncio
@pytest.mark.integration
async def test_profile_nonexistent_dataset_returns_404(
    authed_client, fresh_db, conversation_owned, mock_worker_pool
):
    """POST profile for nonexistent dataset should return 404."""
    from app.main import app
    ...
    fake_id = str(uuid4())
    response = await authed_client.post(
        f"/conversations/{conversation_owned['id']}/datasets/{fake_id}/profile",
    )
...
@pytest.mark.asyncio
@pytest.mark.integration
async def test_profile_column_with_invalid_column_returns_500(
    authed_client, fresh_db, conversation_owned, dataset_in_conversation, mock_worker_pool
):
    """POST profile-column with a column name not in the dataset should return the worker error."""
    from app.main import app

    mock_worker_pool.profile_column = AsyncMock(
        return_value={"error": "Column 'nonexistent_col' not found"}
    )
    app.state.worker_pool = mock_worker_pool

    dataset_id = dataset_in_conversation["id"]
    response = await authed_client.post(
        f"/conversations/{conversation_owned['id']}/datasets/{dataset_id}/profile-column",
        json={"column_name": "nonexistent_col", "column_type": "TEXT"},
    )
...
# ===========================================================================
# 3. Preview with different sampling modes
# ===========================================================================
