...
class TestPoolLifecycle:
    ...
    def test_pool_starts_with_default_size(self):
        ...
        try:
            # Pool should have the requested number of workers
    ...
    def test_pool_can_execute_tasks(self):
        ...
        pool = start(pool_size=2)
        try:
            # Submit a simple task to verify workers are functional
            result = pool.apply(pow, (2, 10))
            assert result == 1024
        finally:
            pool.terminate()
            pool.join()
    ...
    def test_shutdown_waits_for_running_tasks(self):
        ...
        async_result = pool.apply_async(time.sleep, (0.5,))
