...
- **Global focus-visible beats per-component focus styles**: A single CSS rule in globals.css (`*:focus-visible { outline: ... }`) gives consistent keyboard focus to every interactive element. Removes the need for per-component `focus:ring-*` Tailwind classes and prevents inconsistencies (e.g., some using `ring-blue-500`, others `ring-accent`).
