...
- **Always clean up physical files on dataset deletion**: Deleting a DB row for an uploaded dataset (`file://` URL) doesn't delete the file on disk. Must explicitly `os.unlink()` the file path, wrapped in try/except so cleanup failures never block deletion.
- **Playwright globalSetup/globalTeardown must be declared in config**: Having the files exist isn't enough — they must be referenced via `globalSetup` and `globalTeardown` keys in `playwright.config.ts` or the seeding server won't start.
