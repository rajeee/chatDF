...
- **Download retry with backoff**: Remote file downloads can fail transiently (DNS, connection reset). Retry 3x with exponential backoff on URLError/OSError but never retry ValueError (size limits are permanent). Re-open temp file with O_WRONLY|O_TRUNC on retry.
- **Standardize timeouts across modules**: HEAD request timeout was 10s in data_worker.py but 30s in file_cache.py — inconsistency caused by separate implementations. Always check for timeout constants across modules when adding network code.
