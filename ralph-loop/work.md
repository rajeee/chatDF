...
- [x] Fix schema viewer in dataset panel: Fixed in iteration 55 (862c95f). The refresh endpoint returned columns nested under `schema.columns` but frontend expected `result.columns`. Also made `parseColumns` handle wrapped format.
